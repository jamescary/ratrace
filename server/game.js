...
var resetVotes = function (game_id) {
        Votes.update({game_id: game_id, direction: 'left'}, {$set: {count: 0}});
        Votes.update({game_id: game_id, direction: 'down'}, {$set: {count: 0}});
        Votes.update({game_id: game_id, direction: 'up'}, {$set: {count: 0}});
        Votes.update({game_id: game_id, direction: 'right'}, {$set: {count: 0}});
};
...
var maxDir = function (game_id) {
    ...
    for (var i = 0; i < 4; i ++) {
        var currVote = Votes.findOne({game_id: game_id, direction: voteDirs[i]});
        if (!currVote) {
            continue;
        }
